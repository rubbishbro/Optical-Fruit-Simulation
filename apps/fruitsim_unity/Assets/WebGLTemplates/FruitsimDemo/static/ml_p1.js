...
(function () {
  ...
  const CONTROLLER = window.FruitsimP1Controller;
  const CATEGORY_PALETTE = ['#205c86', '#b45f38', '#3e7652', '#7b5ea7', '#9b4b3d', '#3e7f8f', '#8a6d3b'];
  const state = {
    bundle: null,
  ...
  function drawSpectrum(matrixBefore, matrixAfter, wavelengths, sampleIndex, mode, caption, fixedYRange) {
    ...
    const values = before.concat(after || []).flat(); const yRange = fixedYRange || extent(values, -1, 1); const mapper = xyMapper(area, Math.min.apply(null, x), Math.max.apply(null, x), yRange[0], yRange[1]);
    ...
    state.hitPoints = [];
    if (state.selectedWavelengthNm !== null) {
      ctx.strokeStyle = PALETTE.red; ctx.setLineDash([4, 4]); ctx.beginPath();
      ctx.moveTo(mapper.x(state.selectedWavelengthNm), area.top); ctx.lineTo(mapper.x(state.selectedWavelengthNm), area.bottom); ctx.stroke(); ctx.setLineDash([]);
    }
    legend((after ? [{ color: PALETTE.blue, label: 'before / raw' }, { color: PALETTE.orange, label: 'after / selected sample' }] : [{ color: PALETTE.blue, label: 'spectra' }]).concat(state.selectedWavelengthNm === null ? [] : [{ color: PALETTE.red, label: `selected ${format(state.selectedWavelengthNm, 0)} nm` }]));
  }
  ...
  function teachingPreprocessingStage() {
    return CONTROLLER.resolvePreprocessingTeachingStage(
      currentStageObject(),
      selectedPreprocessingComparison(),
      state.animation.pipeline,
    );
  }

  function drawPreprocessEvent(event, item, progress) {
    if (!item || !item.arrays) { drawPreprocessing('overlay'); return; }
    const raw = item.arrays.raw, target = item.arrays.normalized || item.arrays.smoothed, sourceStage = teachingPreprocessingStage(), selectedVisual = sourceStage && sourceStage.visual ? sourceStage.visual : selectedPreprocessingVisual(), x = selectedVisual.wavelengths, sampleIndex = selectedVisual.sample_ids.indexOf(state.selectedSampleId), tween = Math.max(0, Math.min(1, progress === undefined ? 1 : progress));
    if (!raw) { drawPreprocessing('overlay'); return; }
    if (event === 'highlight_sample') {
      drawSpectrum(raw, null, x, sampleIndex, 'overlay', 'Preprocessing teaching step · raw selected sample');
      return;
    }
    if (!target) { drawPreprocessing('overlay'); return; }
    ...
    if (event === 'morph_curve') {
      ...
      const fixedYRange = CONTROLLER.interpolationAxisRange(arr(raw, 'preprocessing raw'), arr(target, 'preprocessing target'), -1, 1);
      drawSpectrum(intermediate, null, x, sampleIndex, 'overlay', `Preprocessing teaching step · raw → output · ${Math.round(tween * 100)}%`, fixedYRange);
      return;
    }
  ...
  function drawPca(mode) {
    ...
    const xValues = scores.map((row) => row[0]), yValues = scores.map((row) => row[1] || 0), xRange = extent(xValues, -1, 1), yRange = extent(yValues, -1, 1), c = canvasContext(), ctx = c.ctx, area = plotArea(c.width, c.height), mapper = xyMapper(area, xRange[0], xRange[1], yRange[0], yRange[1]); axes(ctx, area, xRange[0], xRange[1], yRange[0], yRange[1], 'PC1 score', 'PC2 score'); title(ctx, 'PCA score plot', `click a point · color by ${state.pcaColorBy}`); const target = arr(visual.target, 'PCA target'), groups = visual.groups || visual.batch || [], categorical = state.pcaColorBy === 'batch' && groups.length ? CONTROLLER.categoricalGroups(groups) : null, tRange = extent(target, 0, 1); state.hitPoints = []; scores.forEach((row, i) => { const px = mapper.x(row[0]), py = mapper.y(row[1] || 0), selected = sampleIds[i] === state.selectedSampleId, groupLabel = categorical ? categorical.labels[i] : null; state.hitPoints.push({ x: px, y: py, sampleId: sampleIds[i] }); ctx.beginPath(); ctx.fillStyle = selected ? PALETTE.orange : (categorical ? CATEGORY_PALETTE[categorical.index.get(groupLabel) % CATEGORY_PALETTE.length] : colorScale(target[i], tRange[0], tRange[1])); ctx.strokeStyle = selected ? PALETTE.ink : '#fff'; ctx.lineWidth = selected ? 2.2 : 1; ctx.arc(px, py, selected ? 6 : 4.5, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); }); legend((categorical ? categorical.unique.map((label, i) => ({ color: CATEGORY_PALETTE[i % CATEGORY_PALETTE.length], label: `group ${label}` })) : [{ color: PALETTE.blue, label: 'low target' }]).concat([{ color: PALETTE.orange, label: 'selected sample' }]));
  }

  function drawCars(mode, stepIndex, progress) {
    const visual = currentStageObject().visual, x = arr(visual.wavelengths, 'wavelengths'), states = visual.states || [], stateIndex = CONTROLLER.resolveStepIndex(stepIndex, states.length), tween = Math.max(0, Math.min(1, progress === undefined ? 1 : progress));
  ...
  const RENDERERS = {
    ...
    highlight_sample: (step) => currentStageObject().stage === 'preprocessing' ? drawPreprocessEvent('highlight_sample', step.state, 1) : drawData('spectrum', step),
  ...
  function eventSteps(stage) {
    ...
    const steps = [];
    const sourceStage = stage.stage === 'preprocessing' ? CONTROLLER.resolvePreprocessingTeachingStage(stage, selectedPreprocessingComparison(), state.animation.pipeline) : stage;
  ...
  function applyStep(index, options) {
    const steps = state.animation.steps; if (!steps.length) return; const config = options || {}, previousProgress = state.animation.transitionProgress; cancelMotion(); state.renderMode = 'teaching'; state.animation.index = Math.max(0, Math.min(index, steps.length - 1)); const step = steps[state.animation.index]; state.animation.transitionProgress = config.animate === false || !isTweenEvent(step.event) ? 1 : CONTROLLER.transitionStartProgress(step.event, previousProgress, config.resume === true); if (state.stageKey === 'feature_analysis' && state.stageMode && state.stageMode.indexOf('cars') === 0) state.chartMode = 'iteration'; if (state.stageKey === 'preprocessing' && step.event === 'morph_curve') state.stageMode = 'overlay'; render(); if (state.animation.transitionProgress < 1) startTween(step); else if (config.advance) queueAdvance();
  }
  function startTween(step) {
    const duration = step.event === 'morph_curve' ? 760 : step.event === 'remove_features' ? 680 : 620, initialProgress = state.animation.transitionProgress, started = performance.now() - initialProgress * duration, manual = !state.animation.playing;
