...
(function () {
  ...
  const state = {
    ...
    animation: { steps: [], index: 0, playing: false, timer: null, pipeline: false, pipelineIndex: 0 },
    hitPoints: []
  };
  ...
  function canvasContext() {
    ...
    const width = Math.max(620, Math.floor(rect.width || 980));
    const height = 520;
  ...
  function drawPreprocessing(mode) {
    const stage = activeStage(), comparison = stage.comparisons.find((item) => item.stage_run_id === state.comparisonId) || stage.comparisons.find((item) => item.method_specs.some((spec) => spec.invocation_id === 'snv')) || stage.comparisons[0];
    const visual = comparison.visual, sampleIndex = visual.sample_ids.indexOf(state.selectedSampleId);
  ...
  function drawPreprocessEvent(event, item) {
    if (!item || !item.arrays) { drawPreprocessing('overlay'); return; }
    const raw = item.arrays.raw, normalized = item.arrays.normalized, x = activeStage().visual.wavelengths, sampleIndex = activeStage().visual.sample_ids.indexOf(state.selectedSampleId);
    ...
    if (event === 'show_formula') { drawSpectrum(raw, normalized, x, sampleIndex, 'overlay', 'SNV teaching step · x′ = (x − μ) / σ'); const c = $('ml-chart').getContext('2d'); c.fillStyle = PALETTE.orange; c.font = '600 20px ui-monospace,monospace'; c.fillText('x′ = (x − μ) / σ', 78, 78); return; }
    if (event === 'morph_curve') { drawSpectrum(raw, normalized, x, sampleIndex, 'overlay', 'SNV teaching step · raw → normalized'); return; }
    drawSpectrum(raw, normalized, x, sampleIndex, 'overlay', 'SNV teaching step · raw spectrum');
  }
  ...
  function drawPca(mode) {
    ...
    if (mode === 'loadings') { const c = canvasContext(), ctx = c.ctx, area = plotArea(c.width, c.height), loadings = arr(visual.loadings, 'PCA loadings'), x = arr(visual.wavelengths, 'wavelengths'), yRange = extent(loadings, -1, 1), mapper = xyMapper(area, Math.min(...x), Math.max(...x), yRange[0], yRange[1]); axes(ctx, area, Math.min(...x), Math.max(...x), yRange[0], yRange[1], 'Wavelength (nm)', 'Loading'); title(ctx, 'PCA loading plot', 'which wavelengths contribute to each principal direction'); loadings[0].forEach((_, component) => line(ctx, x.map((value, i) => [value, loadings[i][component]]), mapper, component === 0 ? PALETTE.blue : component === 1 ? PALETTE.orange : PALETTE.gold, 1.8)); legend(loadings[0].map((_, i) => ({ color: i === 0 ? PALETTE.blue : i === 1 ? PALETTE.orange : PALETTE.gold, label: `PC${i + 1} loading` }))); return; }
    const xValues = scores.map((row) => row[0]), yValues = scores.map((row) => row[1] || 0), xRange = extent(xValues, -1, 1), yRange = extent(yValues, -1, 1), c = canvasContext(), ctx = c.ctx, area = plotArea(c.width, c.height), mapper = xyMapper(area, xRange[0], xRange[1], yRange[0], yRange[1]); axes(ctx, area, xRange[0], xRange[1], yRange[0], yRange[1], 'PC1 score', 'PC2 score'); title(ctx, 'PCA score plot', 'click a point to keep the same sample selected across stages'); const target = arr(visual.target, 'PCA target'), tRange = extent(target, 0, 1); state.hitPoints = []; scores.forEach((row, i) => { const px = mapper.x(row[0]), py = mapper.y(row[1] || 0), selected = sampleIds[i] === state.selectedSampleId; state.hitPoints.push({ x: px, y: py, sampleId: sampleIds[i] }); ctx.beginPath(); ctx.fillStyle = selected ? PALETTE.orange : colorScale(target[i], tRange[0], tRange[1]); ctx.strokeStyle = selected ? PALETTE.ink : '#fff'; ctx.lineWidth = selected ? 2.2 : 1; ctx.arc(px, py, selected ? 6 : 4.5, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); }); legend([{ color: PALETTE.blue, label: 'low target' }, { color: PALETTE.orange, label: 'selected sample / high target' }]);
  }

  function drawCars(mode, stepIndex) {
    const visual = currentStageObject().visual, x = arr(visual.wavelengths, 'wavelengths'), states = visual.states || [], stateIndex = Math.max(0, Math.min(states.length - 1, stepIndex || states.length - 1));
    ...
    const selected = states.length ? arr(states[stateIndex].arrays.retained_indices, 'retained indices') : arr(visual.selected_indices, 'selected indices'); const c = canvasContext(), ctx = c.ctx, area = plotArea(c.width, c.height), yRange = [0, 1], mapper = xyMapper(area, Math.min(...x), Math.max(...x), yRange[0], yRange[1]); axes(ctx, area, Math.min(...x), Math.max(...x), yRange[0], yRange[1], 'Wavelength (nm)', 'retained / importance'); title(ctx, `CARS iteration ${stateIndex + 1}`, `${selected.length} wavelengths retained; removed wavelengths fade out`); const retainedSet = new Set(selected); x.forEach((value, i) => { const keep = retainedSet.has(i), importance = states.length ? number(arr(states[stateIndex].arrays.coefficients, 'coefficients')[selected.indexOf(i)] || 0, 0) : 0; ctx.strokeStyle = keep ? PALETTE.blue : '#cbd2d6'; ctx.lineWidth = keep ? 2.5 : 1; ctx.beginPath(); ctx.moveTo(mapper.x(value), area.bottom); ctx.lineTo(mapper.x(value), area.bottom - Math.min(.92, keep ? .18 + importance * 2.4 : .04) * area.height); ctx.stroke(); }); legend([{ color: PALETTE.blue, label: 'retained wavelength' }, { color: '#cbd2d6', label: 'removed wavelength' }]);
  }
  ...
  function drawModel(mode) {
    ...
    if (mode === 'scores') { const scores = arr(visual.scores, 'PLSR scores'), xVals = scores.map((row) => row[0]), yVals = scores.map((row) => row[1] || 0), xRange = extent(xVals, -1, 1), yRange = extent(yVals, -1, 1), mapper = xyMapper(area, xRange[0], xRange[1], yRange[0], yRange[1]); axes(ctx, area, xRange[0], xRange[1], yRange[0], yRange[1], 'latent score 1', 'latent score 2'); title(ctx, 'PLSR latent scores', 'the selected feature matrix is expressed in model directions'); scores.forEach((row, i) => { if (split[i] === 'unused') return; const selected = ids[i] === state.selectedSampleId; ctx.beginPath(); ctx.fillStyle = split[i] === 'validation' ? PALETTE.orange : PALETTE.blue; ctx.arc(mapper.x(row[0]), mapper.y(row[1] || 0), selected ? 6 : 4, 0, Math.PI * 2); ctx.fill(); }); legend([{ color: PALETTE.blue, label: 'calibration' }, { color: PALETTE.orange, label: 'validation' }]); return; }
    if (mode === 'coefficients') { const coefficients = arr(visual.coefficients, 'PLSR coefficients'), xRange = [0, coefficients.length - 1], yRange = extent(coefficients, -1, 1), mapper = xyMapper(area, xRange[0], xRange[1], yRange[0], yRange[1]); axes(ctx, area, xRange[0], xRange[1], yRange[0], yRange[1], 'selected feature index', 'coefficient'); title(ctx, 'PLSR regression coefficients', 'each selected wavelength contributes to the prediction mapping'); line(ctx, coefficients.map((value, i) => [i, value]), mapper, PALETTE.blue, 2); legend([{ color: PALETTE.blue, label: 'coefficient' }]); return; }
    const indicesForPlot = indices.length ? indices : split.map((_, i) => i); const xRange = extent(indicesForPlot.map((i) => yTrue[i]), 0, 1), yRange = extent(indicesForPlot.map((i) => yPred[i]), 0, 1), mapper = xyMapper(area, xRange[0], xRange[1], yRange[0], yRange[1]); axes(ctx, area, xRange[0], xRange[1], yRange[0], yRange[1], 'True target (°Brix)', 'Predicted target (°Brix)'); title(ctx, `Predicted versus true · ${state.stageMode}`, 'validation is the default view; calibration is shown only when selected'); ctx.strokeStyle = PALETTE.muted; ctx.setLineDash([5, 4]); ctx.beginPath(); ctx.moveTo(mapper.x(xRange[0]), mapper.y(xRange[0])); ctx.lineTo(mapper.x(xRange[1]), mapper.y(xRange[1])); ctx.stroke(); ctx.setLineDash([]); indicesForPlot.forEach((i) => { const px = mapper.x(yTrue[i]), py = mapper.y(yPred[i]), selected = ids[i] === state.selectedSampleId; state.hitPoints.push({ x: px, y: py, sampleId: ids[i] }); ctx.beginPath(); ctx.fillStyle = selected ? PALETTE.orange : (split[i] === 'validation' ? PALETTE.blue : PALETTE.blueLight); ctx.strokeStyle = selected ? PALETTE.ink : '#fff'; ctx.lineWidth = selected ? 2 : 1; ctx.arc(px, py, selected ? 6 : 4.5, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); }); legend([{ color: PALETTE.blue, label: state.stageMode }, { color: PALETTE.orange, label: 'selected sample' }, { color: 'transparent', label: 'dashed ideal 1:1' }]);
  }
  ...
  const RENDERERS = { show_spectrum: drawData, highlight_sample: drawData, morph_curve: drawPreprocessing, show_mean: drawPreprocessing, show_std: drawPreprocessing, show_formula: drawPreprocessing, project_points: drawPca, highlight_loading: drawPca, remove_features: drawCars, select_features: drawSelection, connect_feature_to_prediction: drawModel, show_prediction: drawModel, show_residual: drawModel, show_metric: drawResults };
  ...
  function explain(stage) {
    const visual = stage.visual || {}, key = stage.stage, target = visual.target ? arr(visual.target, 'target') : [], metrics = visual.metrics || {};
    const text = {
      ...
      preprocessing: { what: '预处理改变光谱的表示形式，同时保留 sample 和 wavelength 对齐。', why: '减小尺度、基线或局部噪声对后续分析的干扰。', how: '按已保存的 MethodSpec 顺序读取 Raw、SG、SNV 等 StageRun 输出。', read: '切换 before/after、difference 和 heatmap，先看形状变化，再看是否改变样本间可比性。', happened: `${(stage.comparisons || []).length} 个处理组合可比较；当前组合为 ${(stage.comparisons || []).find((item) => item.stage_run_id === state.stageMode)?.method_specs.map((spec) => spec.invocation_id).join(' → ') || 'SNV'}。`, interpretation: '预处理不是增加信息；它只是在表示层面重新组织已有信号。' },
  ...
  function controlsForStage(stage) {
    ...
    if (state.stageKey === 'data_inspection') { container.appendChild(button('Spectra', 'spectrum', state.stageMode === 'spectrum')); container.appendChild(button('Heatmap', 'heatmap', state.stageMode === 'heatmap')); }
  ...
  function stageTabs() { const tabs = $('ml-stage-tabs'); tabs.innerHTML = ''; state.bundle.pipeline_order.forEach((key) => { const button = document.createElement('button'); button.dataset.stageKey = key; button.textContent = STAGE_LABELS[key]; button.onclick = () => { stopAnimation(); state.stageKey = key; state.stageMode = key === 'feature_analysis' ? 'pca' : key === 'preprocessing' ? 'overlay' : key === 'modeling' ? 'validation' : 'spectrum'; state.chartMode = key === 'modeling' ? 'prediction' : key === 'feature_analysis' ? 'scores' : key === 'preprocessing' ? 'overlay' : key === 'feature_analysis' ? 'scores' : 'default'; render(); }; tabs.appendChild(button); }); }
  ...
  function render() {
    ...
    try {
      ...
      else if (state.stageKey === 'preprocessing') { const step = state.animation.steps[state.animation.index]; if (step && ['show_mean', 'show_std', 'show_formula', 'morph_curve'].indexOf(step.event) >= 0 && step.state.arrays.normalized) drawPreprocessEvent(step.event, step.state); else drawPreprocessing(state.chartMode || 'overlay'); }
      else if (state.stageKey === 'feature_analysis') { if (state.stageMode === 'pca' || state.stageMode.indexOf('pca') === 0) { state.chartMode = state.chartMode || 'scores'; drawPca(state.chartMode); } else { state.chartMode = state.chartMode || 'iteration'; drawCars(state.chartMode === 'rmsecv' ? 'rmsecv' : 'iteration', state.animation.index); } }
      else if (state.stageKey === 'feature_selection') drawSelection();
      else if (state.stageKey === 'modeling') drawModel(state.chartMode || 'prediction');
      else if (state.stageKey === 'results') drawResults();
      updateTimeline(); renderResultsOverview();
    } catch (error) { showError(error); }
  }

  function showError(error) { const box = $('ml-chart-error'); box.hidden = false; box.textContent = `图表数据不可用：${error.message || error}`; }
  function hideError() { $('ml-chart-error').hidden = true; }

  function updateTimeline() { const stage = currentStageObject(), steps = eventSteps(stage); state.animation.steps = steps; const slider = $('ml-timeline'); slider.max = Math.max(0, steps.length - 1); slider.value = Math.min(state.animation.index, Math.max(0, steps.length - 1)); setText('ml-timeline-label', `Step ${steps.length ? state.animation.index + 1 : 0} / ${steps.length}`); const item = steps[state.animation.index] || steps[0]; setText('ml-event-label', `event: ${item ? item.event : '-'}`); setText('ml-event-narration', item ? item.narration : '当前阶段没有可播放事件。'); }

  function applyStep(index) { const steps = state.animation.steps; if (!steps.length) return; state.animation.index = Math.max(0, Math.min(index, steps.length - 1)); const step = steps[state.animation.index]; if (state.stageKey === 'feature_analysis' && state.stageMode && state.stageMode.indexOf('cars') === 0) state.chartMode = 'iteration'; if (state.stageKey === 'preprocessing' && step.event === 'morph_curve') state.stageMode = 'overlay'; render(); }
  function stopAnimation() { if (state.animation.timer) window.clearInterval(state.animation.timer); state.animation.timer = null; state.animation.playing = false; state.animation.pipeline = false; setText('ml-pause', '▶ Play'); }
  function playAnimation(pipeline) { stopAnimation(); state.animation.pipeline = !!pipeline; state.animation.playing = true; setText('ml-pause', 'Ⅱ Pause'); state.animation.timer = window.setInterval(() => { const steps = state.animation.steps; if (state.animation.index < steps.length - 1) applyStep(state.animation.index + 1); else if (state.animation.pipeline && state.animation.pipelineIndex < state.bundle.pipeline_order.length - 1) { state.animation.pipelineIndex += 1; state.stageKey = state.bundle.pipeline_order[state.animation.pipelineIndex]; state.stageMode = state.stageKey === 'feature_analysis' ? 'pca' : state.stageKey === 'preprocessing' ? 'overlay' : state.stageKey === 'modeling' ? 'validation' : 'spectrum'; state.chartMode = state.stageKey === 'feature_analysis' ? 'scores' : state.stageKey === 'modeling' ? 'prediction' : 'default'; state.animation.index = 0; render(); } else stopAnimation(); }, 1800); }

  function renderResultsOverview() { const root = $('ml-results-overview'); if (!root || !state.bundle) return; const descriptor = activeDescriptor(), metrics = descriptor.final_metrics || {}; const pipeline = state.bundle.pipeline_order.map((key) => { const stage = key === 'feature_analysis' ? stageItems()[0] : activeStageForKey(key); return stage ? `<button data-jump="${key}">${STAGE_LABELS[key]}</button>` : ''; }).filter(Boolean).join('<b>→</b>'); const experiments = state.bundle.experiments.map((item) => { const m = item.final_metrics || {}; return `<tr><td>${item.experiment_id}</td><td>${item.final_model_id || '-'}</td><td>${format(m.rmse)}</td><td>${format(m.r2)}</td><td>${item.selected_feature_count || '-'}</td></tr>`; }).join(''); root.innerHTML = `<article class="sheet ml-results-card"><div><h3>Experiment pipeline · ${descriptor.experiment_id}</h3><div class="ml-pipeline-links">${pipeline}</div><h3 style="margin-top:18px">Existing ExperimentRun comparison</h3><table class="ml-compare-table"><thead><tr><th>Experiment</th><th>Final invocation</th><th>Validation RMSE</th><th>Validation R²</th><th>Selected features</th></tr></thead><tbody>${experiments}</tbody></table></div><div><h3>Final validation metrics</h3><div class="ml-metric-grid"><div><span>RMSE</span><strong>${format(metrics.rmse)}</strong></div><div><span>MAE</span><strong>${format(metrics.mae)}</strong></div><div><span>R²</span><strong>${format(metrics.r2)}</strong></div><div><span>CV RMSE</span><strong>${format(metrics.cv_rmse)}</strong></div><div><span>bias</span><strong>${format(metrics.bias)}</strong></div><div><span>collapse ratio</span><strong>${format(state.bundle.stages.results.visual.collapse_ratio)}</strong></div></div></div></article>`; root.querySelectorAll('[data-jump]').forEach((button) => button.onclick = () => { state.stageKey = button.dataset.jump; state.stageMode = state.stageKey === 'feature_analysis' ? 'pca' : state.stageKey === 'preprocessing' ? 'overlay' : state.stageKey === 'modeling' ? 'validation' : 'default'; state.chartMode = state.stageKey === 'modeling' ? 'prediction' : state.stageKey === 'feature_analysis' ? 'scores' : 'default'; render(); }); }

  function bind() {
    $('ml-sample-select').onchange = (event) => { state.selectedSampleId = event.target.value; render(); };
    $('ml-feature-select').onchange = (event) => { state.selectedFeatureIndex = Number(event.target.value); render(); };
    $('ml-restart').onclick = () => { stopAnimation(); state.animation.index = 0; render(); };
    $('ml-previous').onclick = () => { stopAnimation(); applyStep(state.animation.index - 1); };
    $('ml-next').onclick = () => { stopAnimation(); applyStep(state.animation.index + 1); };
    $('ml-pause').onclick = () => { if (state.animation.playing) stopAnimation(); else playAnimation(false); };
    $('ml-play-pipeline').onclick = () => { state.animation.pipelineIndex = state.bundle.pipeline_order.indexOf(state.stageKey); state.animation.index = 0; render(); playAnimation(true); };
    $('ml-timeline').oninput = (event) => { stopAnimation(); applyStep(Number(event.target.value)); };
    $('ml-chart').onclick = (event) => { const rect = event.target.getBoundingClientRect(); const x = (event.clientX - rect.left) * 980 / rect.width, y = (event.clientY - rect.top) * 520 / rect.height; let nearest = null, distance = Infinity; state.hitPoints.forEach((point) => { const d = Math.hypot(point.x - x, point.y - y); if (d < distance) { nearest = point; distance = d; } }); if (!nearest || distance > 18) return; if (nearest.sampleId) { state.selectedSampleId = nearest.sampleId; $('ml-sample-select').value = nearest.sampleId; } if (nearest.featureIndex !== undefined) { state.selectedFeatureIndex = nearest.featureIndex; $('ml-feature-select').value = nearest.featureIndex; } render(); };
    window.addEventListener('resize', () => { if (state.bundle && document.querySelector('#page-ml').classList.contains('active')) render(); });
  }

    function loadBundle(bundle) { state.bundle = decode(bundle); state.activeExperimentId = state.bundle.experiment_id; state.comparisonExperimentId = state.bundle.experiment_id; state.selectedSampleId = state.bundle.selected_sample_id; const sampleSelect = $('ml-sample-select'), featureSelect = $('ml-feature-select'), data = state.bundle.stages.data_inspection.visual; arr(data.sample_ids, 'sample ids').forEach((id) => { const option = document.createElement('option'); option.value = id; option.textContent = id; sampleSelect.appendChild(option); }); sampleSelect.value = state.selectedSampleId; arr(data.wavelengths, 'wavelengths').forEach((value, index) => { const option = document.createElement('option'); option.value = index; option.textContent = `${value} nm`; featureSelect.appendChild(option); }); state.comparisonId = state.bundle.stages.preprocessing.comparisons.find((item) => item.method_specs.some((spec) => spec.invocation_id === 'snv')).stage_run_id; const expSelect = $('ml-experiment-select'); state.bundle.experiments.forEach((item) => { const option = document.createElement('option'); option.value = item.experiment_id; option.textContent = item.experiment_id === state.bundle.experiment_id ? `${item.experiment_id} · active` : `${item.experiment_id} · comparison`; expSelect.appendChild(option); }); expSelect.value = state.comparisonExperimentId; expSelect.onchange = () => { state.comparisonExperimentId = expSelect.value; const isComparison = state.comparisonExperimentId !== state.bundle.experiment_id; setText('ml-play-status', isComparison ? 'comparison shown in Results table; stage detail remains active' : 'ready'); render(); }; setText('ml-source-tag', `${state.bundle.source.source_type.toUpperCase()} · ${state.bundle.source.sample_count} samples`); stageTabs(); bind(); render(); }
